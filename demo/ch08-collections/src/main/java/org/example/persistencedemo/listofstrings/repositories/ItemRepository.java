package org.example.persistencedemo.listofstrings.repositories;
...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    org.example.persistencedemo.listofstrings.domain.Item findItemWithImages(@Param("id") Long id);

    @Query(value = "SELECT FILENAME FROM image WHERE ITEM_ID = ?1", nativeQuery = true)
    List<String> findImagesNative(Long id);
}
